...
function UsBuilder() {
  ...
  const sel = useListSelection(US_SORTED, LIMITS.targetSchools, (u) => u.name, usSearch);
        ...
        return {
          ...
          badge: branch ? (
            <span className="mt-1.5 inline-flex w-fit items-center gap-1.5 rounded-full bg-accent-soft px-2 py-0.5 text-[10px] font-semibold text-accent-ink">
              ...
              Campus in {branch.host_country.replace(/^the /, "")} · US-system admissions
            </span>
          ) : undefined,
          meta: [
...
const itSearch = (p: (typeof IT_SORTED)[number], q: string) =>
  lc(p.university).includes(q) || lc(p.program_name).includes(q) || lc(p.city).includes(q);

function ItalyBuilder() {
  const { profileMeta } = useDashboard();
  const ff = useFieldFilter(IT_SORTED, fieldOf, profileMeta.faculties, italianFieldsForFaculties);
  const sel = useListSelection(ff.list, LIMITS.italyPrograms, (p) => p.id, itSearch);
...
const hkSearch = (p: (typeof HK_SORTED)[number], q: string) =>
  lc(p.university).includes(q) || lc(p.program_name).includes(q) || lc(p.field).includes(q);

function HkBuilder() {
  const { profileMeta } = useDashboard();
  const ff = useFieldFilter(HK_SORTED, fieldOf, profileMeta.faculties, programFieldsForFaculties);
  const sel = useListSelection(ff.list, LIMITS.hkPrograms, (p) => p.id, hkSearch);
  ...
  return (
    <ListBuilderShell
      ...
      controls={
        <>
          <FieldFilter ff={ff} faculties={profileMeta.faculties} />
          <GradeStatusField value={gradeStatus} onChange={setGradeStatus} className="max-w-md" />
        </>
      }
      card={(p) => ({
        title: p.university,
...
function UaeBuilder() {
  const { profileMeta } = useDashboard();
  const ff = useFieldFilter(AE_SORTED, fieldOf, profileMeta.faculties, programFieldsForFaculties);
  const sel = useListSelection(ff.list, LIMITS.uaePrograms, (p) => p.id, aeSearch);
  const [gradeStatus, setGradeStatus] = useState<"predicted" | "achieved">("predicted");
...
function KoreaBuilder() {
  const { profileMeta } = useDashboard();
  const ff = useFieldFilter(KR_SORTED, fieldOf, profileMeta.faculties, programFieldsForFaculties);
  const sel = useListSelection(ff.list, LIMITS.krPrograms, (p) => p.id, krSearch);
  const [gradeStatus, setGradeStatus] = useState<"predicted" | "achieved">("predicted");
  ...
  const run = () =>
    submit(async () => {
      ...
      return saveKoreaList(sel.selected, gradeStatus, topik > 0 ? topik : undefined);
    });
