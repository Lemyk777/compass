...
import type { ProfileMeta } from "@/components/dashboard/DashboardContext";

// Everything a signed-in student's surfaces need, fetched once.
//
// This used to live inline in app/dashboard/layout.tsx, back when the report
// WAS the product and every student page hung off it. Opportunities is now its
// own top-level section with its own shell, and it needs exactly the same facts
// (profile, live dates, what they've committed to), so the loader moved here
// rather than being copied. One query set, two shells.
...
export async function loadStudentContext(
  ...
  const [
    ...
    supabase
      ...
      .limit(1)
      .maybeSingle(),
    // Select * so a DB missing newer columns still returns the row.
    supabase
    ...
    // Timeline falls back to hardcoded dates in key-dates.ts.
    supabase
      .from("sat_sittings")
    ...
    // 0022). select("*") on purpose, not an explicit column list: why_matters
    // arrives in 0023, and naming a not-yet-created column would fail the whole
    // select and blank out every committed intent until it is applied.
    supabase
  ...
  const satSittings: SatSitting[] = (satRows ?? []).map(
    (r: { test_date: string; reg_deadline: string }) => ({
      test: r.test_date,
      regDeadline: r.reg_deadline,
    }),
  );

  // Raw live rows; resolveCompetitions keeps the curated code authoritative and
  // overlays a live date ONLY when the scraper marked it confirmed. The mapping
  // lives in lib/partners/live.ts because the public eligibility checker needs
  // exactly the same rules (including "a suspended partner's posts are gone").
  const competitions: Competition[] = competitionsFromRows(
  ...
  return {
    analysis,
    hasProfile,
    destinations: Array.isArray(sp?.destinations)
      ? (sp!.destinations as DestinationCode[])
      : [],
    profileMeta: {
