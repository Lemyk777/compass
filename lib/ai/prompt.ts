...
In the "reasoning" array, provide 2-4 specific bullet points that:
  (a) cite the EXACT evidence from the profile (names, roles, outcomes, grades, test scores) that justifies this tier,
  (b) weigh that evidence against the rubric's tier boundary — explain concretely why it lands at THIS tier and not the one above (which part of the higher bar it meets and which it falls short of), unless you gave the top tier.
This keeps the score defensible and transparent: give enough specific detail that the student understands exactly WHY the number is what it is. Never give a vague justification like "strong profile" — always point to specific data. (Do NOT add what-to-do-next advice in the reasoning — that lives in the gap analysis and timeline.)
...
- extracurricular_depth: identify the student's STRONGEST sustained activity (their "spike"), then place THE PROFILE in a tier. Tier 1 splits — reserve 10 for pre-professional level: published research in a peer-reviewed venue, recruited athlete, professional-level performer, or a venture with real revenue/users at scale. Award 9 for an elite selective program (e.g. top-university summer school on scholarship/grant, RSI, MOSTEC, TASP), OR top international placement in a competitive field with tangible outcomes (seed funding, international finals, paid delegate invitations), OR a sustained multi-year spike (3-4 grades) combining depth, international exposure, and real-world output (products shipped, organizations built, competitions won). Tier 2 (7-8): a clear, sustained 2-3 year focus with meaningful projects and outputs, but without the international-elite markers of Tier 1. Tier 3 (5-6): standard school clubs, moderate involvement. Tier 4 (2-4): shallow, scattered, nominal involvement. Reward DEPTH — seniority of role, how many grade levels it is sustained across, selectivity, international exposure, and tangible output — NOT the count of activities. The intake does not collect hours; never expect or require an hours figure — judge commitment from the role, organization, the grade levels the activity spans, and the outcomes.
...
For all three, state the chosen tier in "rubric_tier" and name the exact activity/award and its level/scale in "reasoning".
