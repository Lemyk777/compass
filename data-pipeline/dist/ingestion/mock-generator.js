...
exports.generateMockRedditPost = generateMockRedditPost;
exports.generateMockTwitterPost = generateMockTwitterPost;
exports.writeMockDataset = writeMockDataset;
...
function generateMockRedditPost(index) {
    ...
    const ecLines = ecs.map((ec, idx) => `${idx + 1}. **${ec.position}** at *${ec.org}*: ${ec.desc} (Grades 10-12, ${Math.floor(Math.random() * 8) + 4} hrs/week)`).join('\n');
    // Honors
    const numHonors = Math.floor(Math.random() * 3) + 1; // 1 to 3
    const honors = getRandomElements(HONORS, numHonors);
    const honorLines = honors.map((h, idx) => `${idx + 1}. **${h.title}** - Level: ${h.levels.join(', ')} (Grade ${Math.floor(Math.random() * 4) + 9})`).join('\n');
...
function generateMockTwitterPost(index) {
    const country = getRandomElement(COUNTRIES);
    const major = getRandomElement(MAJORS);
    const curriculum = getRandomElement(CURRICULUMS);
    let gpaStr = '';
...
function writeMockDataset(rawDir, count = 100) {
